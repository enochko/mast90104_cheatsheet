...
%    {\geometry{top=0.75cm,left=0.2cm,right=0.2cm,bottom=0.75cm}}
...
$\mathbb E [\mathbf{y}^T \mathbf{A}\mathbf{y}] = \mathrm{tr}(\mathbf{AV}) + \boldsymbol\mu^T \mathbf A\boldsymbol\mu$.\\
\medskip
...
\medskip
\textbf{Leverage:} $h_{ii} = \frac{\partial \widehat{y}_i}{\partial y_i} = [\mathbf{H}]_{ii}$ \; ($0 < h_{ii} < 1$). Self-sensitivity.\\ 
\textbf{Residual:} $e_i = y_i - \widehat{y}_i$.\quad
\textbf{Var(residual):} $\mathrm{Var}(e_i) = \sigma^2 (1 - h_{ii})$. \\
\textbf{Standardised residual:}  $z_i = \frac{e_i}{s \sqrt{1-h_{ii}}}$; $s^2 = \tfrac{\mathrm{SSE}}{n-p}$. $\mathrm{Var}(z_i)=1$.\\
...
$Z=\frac{\widehat{\boldsymbol\beta}_i-\boldsymbol\beta_i}{\sigma\sqrt{c_{ii} } }\sim N(0,1)$;\qquad$X^2=\frac{(n-p)s^2}{\sigma^2}=\frac{\mathrm{SS}_\mathrm{res}}{\sigma^2}\sim\chi^2_{n-p}$;\\
...
Probability CI contains the true population param is $1-\alpha$.
...
$t^T \widehat{\boldsymbol\beta}$: $\text{CI} = t^T \widehat{\boldsymbol\beta}\pm t_{\alpha/2} s\sqrt{(t^T(\mathbf X^T\mathbf X)^{-1}t}$\\
...
$\sigma^2:\text{CI}=\left[\frac{(n-p)s^2}{\chi^2_{1-\alpha/2,\,n-p}}\;,\;\frac{(n-p)s^2}{\chi^2_{\alpha/2,\,n-p}}\right]$\\
\medskip
...
$\mathrm{SS}_\mathrm{Res}=\mathrm{SSE}=(\mathbf y-\mathbf X\widehat{\boldsymbol\beta})^T(\mathbf y-\mathbf X\widehat{\boldsymbol\beta})=\mathbf y^T(\mathbf I-\mathbf H)\mathbf y$.\\
...
\medskip
Under $H_0$: $\mathrm{SS}_\mathrm{Res}^{H_0}=\mathrm{SS}_\mathrm{Total}$; $\mathrm{SS}_\mathrm{Reg}^{H_0}=0$.\\
\medskip
...
$\mathrm{SS}_\mathrm{Res}^{H_0}=\mathbf y^T\mathbf y-\frac{1}{n}\mathbf y^T\boldsymbol1\boldsymbol1^T\mathbf y$. (Corrected sum of squares, CSS)\\
$\mathrm{SS}_\mathrm{Reg}^{H_0}=\frac{1}{n}\mathbf y^T\boldsymbol1\boldsymbol1^T\mathbf y$. (Correction factor)\\
\medskip
...
If $y=c+x_1+x_2+x_3$;
$H_0:\beta_1=\beta_2=0$;
$\mathbf L=\begin{bmatrix}0&1&0&0\\0&0&1&0\end{bmatrix}$; $\boldsymbol\delta=\begin{bmatrix}0\\0\end{bmatrix}$\\
$H_0:\beta_1=\beta_2$;
$\mathbf L=\begin{bmatrix}0&1&-1&0\end{bmatrix}$; $\boldsymbol\delta=\begin{bmatrix}0\end{bmatrix}$\\
\medskip
\medskip
...
1. $\mathrm{SS}_{\mathrm{Res}}$: Prefer lower. Always decrease w/more predictors.\\
2. $\mathrm{MSE}/s^2$: Prefer lower. Penalises more complex models by n-p.\\
3. $R^2=1-\frac{\mathrm{SS}_{\mathrm{Res}}}{\mathrm{SS}_{\mathrm{Total}}-(\sum_iy_i)^2/n)}=1-\frac{\mathrm{SS}_{\mathrm{Res}}}{CSS}$: Prefer higher. Nested models: always pick most complicated model like $\mathrm{SS}_{\mathrm{Res}}$.\\
4. $\mathrm{Adj-}R^2=1-\frac{n-1}{n-1-k}(1-R^2)=1-\frac{n-1}{n-p}(1-R^2)=1-\frac{n-1}{n-p}\cdot\frac{\mathrm{SS}_{\mathrm{Res}}}{\mathrm{CSS}}$: Prefer lower. Penalise large no. of slopes via n-1-k.\\
5. $\mathrm{AIC}=-2 \ln(L)+2p=n\ln(\frac{\mathrm{SS}_{\mathrm{Res}}}{n})+2p+const.$: Prefer lower.\\
6. Mallow's $C_p=\frac{\mathrm{SS}_{\mathrm{Res}}(\text{model})}{s^2(\text{full model})}+2p-n$: Prefer lower.
p=param in intermediate model.\\
\medskip
...
\medskip
\subsection{Proofs}
\subsubsection{Idempotent matrix eigenvalue 0 or 1:}
$\mathbf{A}\mathbf{x}=\lambda\mathbf{x}$.
$\mathbf{A}^2\mathbf{x}=\mathbf{A}\lambda\mathbf{x}=\lambda\mathbf{A}\mathbf{x}=\lambda^2\mathbf{x}$.
$\Rightarrow$$\lambda^2\mathbf{x}=\mathbf{A}^2\mathbf{x}=\mathbf{A}\mathbf{x}=\lambda\mathbf{x}$.
$\Rightarrow$$(\lambda^2-\lambda)\mathbf{x}=\boldsymbol 0$.
$\Rightarrow$$\lambda=\lambda^2$ ($\because \mathbf{x}\neq\boldsymbol 0$).
$\Rightarrow$$\lambda=0\text{ or }1$.\\
\medskip
\subsubsection{Idempotent matrix rank=trace:}
$\mathbf{A}$ idemp.$\Rightarrow$$\lambda=0\text{ or }1$.
Sym. idemp. $\Rightarrow$ $\exists$ ortho $\mathbf{P}$ diag $\mathbf{A}$. $\mathrm{r}(\mathbf{P}^T\mathbf{A}\mathbf{P})=\mathrm{r}(\mathbf{A})=$non-zero $\lambda$ of $\mathbf{A}$.
$\mathrm{r}(\mathbf{A})=\mathrm{r}(\mathbf{P}^T\mathbf{A}\mathbf{P})=\mathrm{tr}(\mathbf{P}^T\mathbf{A}\mathbf{P})=\mathrm{tr}(\mathbf{P}\mathbf{P}^T\mathbf{A})=\mathrm{tr}(\mathbf{A})$.\\
\medskip
\subsubsection{Expectation of quadratic form:}
$\mathbb E [\mathbf y^T \mathbf{Ay}]=\mathbb E [\mathrm{tr}(\mathbf y^T \mathbf{Ay})]=\mathbb E [\mathrm{tr}(\mathbf{y}\mathbf y^T \mathbf{A})]=\mathrm{tr}[\mathbb E(\mathbf{y}\mathbf y^T \mathbf{A})]$\\
$\because \mathrm{Var}(\mathbf y)=\mathbb E [(\mathbf y-\boldsymbol\mu)(\mathbf y-\boldsymbol\mu)^T]=\mathbb E (\mathbf y\mathbf y^T)-\boldsymbol\mu\boldsymbol\mu^T=\mathbf{V}$\\
$\therefore\mathbb E [\mathbf y^T \mathbf{Ay}]=\mathrm{tr}[(\mathbf{V}+\boldsymbol\mu\boldsymbol\mu^T)\mathbf{A})]= \mathrm{tr}[\mathbf{V}\mathbf{A}+\boldsymbol\mu\boldsymbol\mu^T\mathbf{A})]=\mathrm{tr}(\mathbf{AV}) + \boldsymbol\mu^T \mathbf A\boldsymbol\mu$.\\
\medskip
\subsubsection{Expectation and Variance of y and beta:}
$\mathbb E(\widehat{\mathbf y})=\mathbf H\,\mathbb E(\mathbf y)=\mathbf X\boldsymbol{\beta}$;
$\mathrm{Var}(\widehat{\mathbf y})=\mathbf H\,\mathrm{Var}(\mathbf y)\,\mathbf H^T=\sigma^2$\\
\medskip
$\mathbb E[\widehat{\boldsymbol{\beta} }]=(\mathbf X^T\mathbf X)^{-1}\mathbf X^T\mathbb E[\mathbf y]=\boldsymbol{\beta}$;
$\mathrm{Var}(\widehat{\boldsymbol{\beta} })=\mathrm{Var}[(\mathbf X^T\mathbf X)^{-1}\mathbf X^T\mathbf y] = \sigma^2(\mathbf X^T\mathbf X)^{-1}$\\
\medskip
\subsubsection{Sigma2 is biased:}
...
\medskip
\subsubsection{CI for variance:}
$\frac{(n-p)s^2}{\sigma^2}=\frac{\mathrm{SS}_\mathrm{res}}{\sigma^2}=\frac{\mathbf y^T(\mathbf I-\mathbf H)\mathbf y}{\sigma^2}\sim\chi^2_{n-p}$
$\mathbb P(\chi^2_{\alpha/2,n-p}\le\frac{(n-p)s^2}{\sigma^2}\le\chi^2_{1-\alpha/2,n-p})=1-\alpha$\\
\medskip
\vfill
Last updated:
2025-09-10 T11-12-39\\
Compiled: \DTMnow\\
\hrule
