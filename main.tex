...
\textbf{Graphs:} Residual vs Fitted$\rightarrow$ linearity / QQ-plot $\rightarrow$ normality / Scale-Location $\rightarrow$ homoscedasticity / Residual vs Leverage (Cook's distance).
...
$\mathbf L:r\times p;\quad \mathrm{r(\mathbf L):r\le p}$;\quad$\boldsymbol\delta:r\times 1$ \\
\medskip
...
\medskip
\textbf{Stepwise Selection:} Start w/any model. Add/remove 1 var based on goodness-of-fit - Akaike's Information Criterion (AIC). Smaller AIC=better. Better than forward/backward. Finds local optimum.\\
\medskip
...
$R^2_{adj}=1-\frac{n-1}{n-1-k}(1-R^2)=1-\frac{n-1}{n-p}(1-R^2)=1-\frac{n-1}{n-p}\cdot\frac{\mathrm{SS}_{\mathrm{Res}}}{\mathrm{SS}_{\mathrm{Total}}}$\\
...
where p=param in intermediate model. Small $C_p$=better.\\
\medskip
