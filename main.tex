...
$\text{bias}=\mathbb E(\widehat{\sigma}^2)-\sigma^2=\mathbb E\left(\frac{\|\mathbf y-\mathbf X\boldsymbol\beta\|^2}{n}\right)-\sigma^2=\frac{n-p}{n}\mathbb E\left(\frac{\|\mathbf y-\mathbf X\boldsymbol\beta\|^2}{n-p}\right)-\sigma^2=\dfrac{n-p}{n}\mathbb E(s^2)-\sigma^2=\dfrac{n-p}{n}\mathbb \sigma^2-\sigma^2=-\dfrac{p}{n}\mathbb \sigma^2$\\
\medskip
...
\textbf{Leverage:} $h_{ii} = \dfrac{\partial \widehat{y}_i}{\partial y_i} = [\mathbf{H}]_{ii}$ 
\; ($0 < h_{ii} < 1$). \\[6pt]
\textbf{Residual:} $e_i = y_i - \widehat{y}_i$. \\[6pt]
\textbf{Var(residual):} $\mathrm{Var}(e_i) = \sigma^2 (1 - h_{ii})$. \\[6pt]
\textbf{Standardised residual:} 
$z_i = \frac{e_i}{s \sqrt{1-h_{ii}}}$, \quad
$s^2 = \tfrac{\mathrm{SSE}}{n-p}$. \\[6pt]
\textbf{Cook's distance:} 
$D_i = \frac{(\widehat{\boldsymbol\beta}_{(-i)}-\widehat{\boldsymbol\beta})^T\mathbf X^T\mathbf X(\widehat{\boldsymbol\beta}_{(-i)}-\widehat{\boldsymbol\beta})}{ps^2} = \frac{z_i^2}{p}\left(\frac{h_{ii} }{1-h_{ii} }\right)$, where $\widehat{\boldsymbol\beta}_{(-i)}$ is the estimated coefficient vector if the point $i$  is removed and $p$ is the size of $\widehat{\boldsymbol\beta}$.
\medskip
...
$Z=\frac{\widehat{\boldsymbol\beta}_i-\boldsymbol\beta_i}{\sigma\sqrt{c_{ii} } }\sim N(0,1)$; $X^2=\frac{\mathrm{SS}_\mathrm{res}}{\sigma^2}\sim\chi^2_{n-p}$;\\
...
$\Rightarrow \left(\frac{(\widehat{\boldsymbol\beta}-\boldsymbol\beta)^T(\mathbf X^T\mathbf X)(\widehat{\boldsymbol\beta}-\boldsymbol\beta)}{\sigma^2}\right)/\left(\frac{(n-p)s^2}{(n-p)\sigma^2}\right)$\\
...
C.R. is ellipse/ellipsoid because LHS is quadratic form whose matrix $\mathbf X^T\mathbf X$ is positive definite.\\
\medskip
\subsection{ANOVA}
